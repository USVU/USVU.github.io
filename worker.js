...
var pyodide = null;
var dlFlag = null, dlData = null, promptFlag = null, promptData = null, interruptFlag = null;
...
self.addEventListener('message', function(e) {
  if (e.data.type === 'init') {
    ...
    promptFlag[0] = 0;
    interruptFlag = new Int32Array(e.data.sab, 8204, 1);
  }
});
...
async function boot() {
  ...
  if (interruptFlag) pyodide.setInterruptBuffer(interruptFlag);
  ...
  pyodide.registerJsModule('_stdin_bridge', {
    read: function() {
      ...
      if (interruptFlag && interruptFlag[0] === 2) {
        interruptFlag[0] = 0;
        dlFlag[0] = 0;
        return '\x03';
      }
  ...
  pyodide.setStdout({
    batched: function(t) {
      if (!t) return;
      if (t.indexOf('\x1b[2J') !== -1) {
        self.postMessage({ type: 'clear' });
        var remaining = t.replace(/\x1b\[2J\x1b\[H/g, '');
        if (remaining.trim()) {
          var c = stripAnsi(remaining);
